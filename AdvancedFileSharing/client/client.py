...
# This will log messages to both the console and a file (client.log)
...
# --- Constants ---
CHUNK_SIZE = 1024  # Amount of bytes to read/write at once
CLIENT_DATA_PATH = "client_data"  # Where downloaded files will be stored

# Make sure the folder for downloaded files exists
...
def calculate_hash(file_bytes):
    """
    Returns the SHA-256 hash of given file bytes.
...
def upload_file(sock):
    ...
    - Sends file name, size, contents, and hash.
...
def download_file(sock):
    ...
    - Verifies the file's hash after downloading.
    - Saves it locally if verification passes.
    """
    ...
    try:
        ...
        # Receive file hash and verify it
...
def list_files(sock):
    """
    Requests and displays a list of available files from the server.
    """
    try:
